...
// extensions modify begin;
            ...
            if (grayScale)
            {
                color32 = new Color(1f, 1f, 1f, color32.a);
                gray = 1;
            }
            vh.Clear();
            vh.AddVert(new Vector3(v.x, v.y), color32, new Vector4(uv.x, uv.y, gray));
            vh.AddVert(new Vector3(v.x, v.w), color32, new Vector4(uv.x, uv.w, gray));
            vh.AddVert(new Vector3(v.z, v.w), color32, new Vector4(uv.z, uv.w, gray));
            vh.AddVert(new Vector3(v.z, v.y), color32, new Vector4(uv.z, uv.y, gray));
...
// extensions modify end;
            ...
            {
// extensions modify begin;
                vh.AddVert(new Vector3((vertices[i].x / spriteBoundSize.x) * drawingSize.x - drawOffset.x, (vertices[i].y / spriteBoundSize.y) * drawingSize.y - drawOffset.y), color32, new Vector3(uvs[i].x, uvs[i].y, gray));
// extensions modify end;
            }
...
// extensions modify begin;
            ...
            if (grayScale)
            {
                color32 = new Color(1f, 1f, 1f, color.a);
                param.x = 1f;
            }
// extensions modify end;
            for (int x = 0; x < 3; ++x)
...
// extensions modify begin;
                            AddQuad(toFill, new Vector2(x1, y1) + rect.position, new Vector2(x2, y2) + rect.position, color32, uvMin, clipped, param);
// extensions modify end;
                        }
                    }
                }
...
// extensions modify end;
            ...
            else
            {
                ...
                {
// extensions modify begin;
                    AddQuad(toFill, new Vector2(xMin, yMin) + rect.position, new Vector2(xMax, yMax) + rect.position, color32, Vector2.Scale(uvMin, uvScale), Vector2.Scale(uvMax, uvScale), param);
// extensions modify end;
                }
            }
        }
...
// extensions modify begin;
        static void AddQuad(VertexHelper vertexHelper, Vector2 posMin, Vector2 posMax, Color32 color, Vector2 uvMin, Vector2 uvMax, Vector2 param)
// extensions modify end;
        {
            int startIndex = vertexHelper.currentVertCount;
// extensions modify begin;
            vertexHelper.AddVert(new Vector3(posMin.x, posMin.y, 0), color, new Vector4(uvMin.x, uvMin.y, param.x, param.y));
            vertexHelper.AddVert(new Vector3(posMin.x, posMax.y, 0), color, new Vector4(uvMin.x, uvMax.y, param.x, param.y));
            vertexHelper.AddVert(new Vector3(posMax.x, posMax.y, 0), color, new Vector4(uvMax.x, uvMax.y, param.x, param.y));
            vertexHelper.AddVert(new Vector3(posMax.x, posMin.y, 0), color, new Vector4(uvMax.x, uvMin.y, param.x, param.y));
// extensions modify end;
            vertexHelper.AddTriangle(startIndex, startIndex + 1, startIndex + 2);
            vertexHelper.AddTriangle(startIndex + 2, startIndex + 3, startIndex);
        }
...
// extensions modify begin;
            ...
            s_Xy[0] = new Vector3(v.x, v.y, gray);
            s_Xy[1] = new Vector3(v.x, v.w, gray);
            s_Xy[2] = new Vector3(v.z, v.w, gray);
            s_Xy[3] = new Vector3(v.z, v.y, gray);

            s_Uv[0] = new Vector3(tx0, ty0, gray);
            s_Uv[1] = new Vector3(tx0, ty1, gray);
            s_Uv[2] = new Vector3(tx1, ty1, gray);
            s_Uv[3] = new Vector3(tx1, ty0, gray);
// extensions modify end;
            {
...
// extensions modify begin;
                            AddQuad(toFill, s_Xy, color32, s_Uv);
// extensions modify end;
                    }
