...
// extensions modify begin;
                ...
                {
                    CoreUtils.SetKeyword(m_Materials.uber, ShaderKeywordStrings.LinearToSRGBConversion, RequireSRGBConversionBlitToBackBuffer(ref cameraData) || cameraData.requireLinearToSRGBConversion);
                    CoreUtils.SetKeyword(m_Materials.uber, ShaderKeywordStrings.SRGBToLinearConversion, cameraData.requireSRGBToLinearConversion);
                }
                else
                {
...
// extensions modify begin;
                    RTHandle dest = (m_MixGammaColorSpace && cameraData.requireLinearToSRGBConversion) ? m_GammaDestination : destination;
                    Blitter.BlitCameraTexture(cmd, GetSource(), dest, colorLoadAction, RenderBufferStoreAction.Store, m_Materials.uber, 0);
                    renderer.ConfigureCameraColorTarget(dest);
// extensions modify end;
                    Swap(ref renderer);
                }
...
// extensions modify begin;
                    RTHandle dest = (m_MixGammaColorSpace && cameraData.requireLinearToSRGBConversion) ? m_GammaDestination : m_Destination;
                    int pass = dest.rt != null && dest.rt.filterMode == FilterMode.Bilinear ? 1 : 0;
                    Blitter.BlitCameraTexture(cmd, GetDestination(), dest, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, m_BlitMaterial, pass);
// extensions modify end;
                }
