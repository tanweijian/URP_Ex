...
// extensions modify begin;
        DrawObjectsGammaSpacePass m_RenderOpaqueColorSpaceGammaForwardPass;
        DrawObjectsGammaSpacePass m_RenderTransparentColorSpaceGammaForwardPass;
        BlitPass m_LinearToSRGBConversion;
        
#if UNITY_EDITOR
        BlitPass m_OpaqueLinearToSRGBConversionSceneView;
        BlitPass m_OpaqueSRGBToLinearConversionSceneView;
        BlitPass m_TransparentLinearToSRGBConversionSceneView;
        BlitPass m_TransparentSRGBToLinearConversionSceneView;
#endif
        RTHandle m_GammaColorTexture;
        bool m_mixGammaColorSpace;
...
// extensions modify begin;
            m_mixGammaColorSpace = data.MixGammaColorSpace;
// extensions modify end;
...
// extensions modify begin;
                if (m_mixGammaColorSpace)
                {
                    m_RenderTransparentColorSpaceGammaForwardPass = new DrawObjectsGammaSpacePass("DrawTransparentObjectsGammaSpace", false, RenderPassEvent.BeforeRenderingTransparents + 2, RenderQueueRange.transparent, data.GammaTransparentLayerMask, m_DefaultStencilState, stencilData.stencilReference);
                }
// extensions modify end;
            }
...
// extensions modify begin;
            if (m_mixGammaColorSpace)
            {
                m_RenderOpaqueColorSpaceGammaForwardPass = new DrawObjectsGammaSpacePass("DrawOpaqueObjectsGammaSpace", true, RenderPassEvent.BeforeRenderingOpaques + 2, RenderQueueRange.opaque, data.GammaOpaqueLayerMask, m_DefaultStencilState, stencilData.stencilReference);
                m_LinearToSRGBConversion = new BlitPass("LinearToSRGBConversion", RenderPassEvent.AfterRendering, m_BlitMaterial, true, false);
#if UNITY_EDITOR
                m_OpaqueLinearToSRGBConversionSceneView = new BlitPass("LinearToSRGBConversion",RenderPassEvent.BeforeRenderingOpaques + 1, m_BlitMaterial, true, false);
                m_OpaqueSRGBToLinearConversionSceneView = new BlitPass("SRGBToLinearConversion",RenderPassEvent.BeforeRenderingOpaques + 3, m_BlitMaterial, false, true);
                m_TransparentLinearToSRGBConversionSceneView = new BlitPass("LinearToSRGBConversion",RenderPassEvent.BeforeRenderingTransparents + 1, m_BlitMaterial, true, false);
                m_TransparentSRGBToLinearConversionSceneView = new BlitPass("SRGBToLinearConversion",RenderPassEvent.BeforeRenderingTransparents + 3, m_BlitMaterial, false, true);
#endif
            }
// extensions modify end;
...
// extensions modify begin;
            if (m_mixGammaColorSpace && ((cameraData.isSceneViewCamera && cameraData.forceGammaColorSpace) || cameraData.requireLinearToSRGBConversion))
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(ref m_GammaColorTexture, descriptor, FilterMode.Bilinear, TextureWrapMode.Clamp, name: "_GammaColorAttachment");
            }
// extensions modify end;
            ...
            {
// extensions modify begin;
                void EnqueueOpaqueGammaPass(ref CameraData cameraData)
                {
                    RTHandle colorAttachment = cameraData.isSceneViewCamera ? m_GammaColorTexture : m_ActiveCameraColorAttachment;
                    m_RenderOpaqueColorSpaceGammaForwardPass.Setup(colorAttachment, m_ActiveCameraDepthAttachment);
                    EnqueuePass(m_RenderOpaqueColorSpaceGammaForwardPass);
                }

                void EnqueueOpaqueNormalPass(ref CameraData cameraData)
                {
                ...
                if (cameraData.isSceneViewCamera)
                {
                    ...
                    if (m_mixGammaColorSpace && cameraData.forceGammaColorSpace)
                    {
                        ...
                        EnqueuePass(m_OpaqueLinearToSRGBConversionSceneView);
                        m_OpaqueSRGBToLinearConversionSceneView.Setup(m_GammaColorTexture, m_ActiveCameraColorAttachment);
                        EnqueuePass(m_OpaqueSRGBToLinearConversionSceneView);
                    }
                }
                else
#endif
...
// extensions modify end;
            ...
            {
// extensions modify begin;
                void EnqueueTransparentGammaPass(ref CameraData cameraData)
                {
                    RTHandle colorAttachment = cameraData.isSceneViewCamera ? m_GammaColorTexture : m_ActiveCameraColorAttachment;
                    m_RenderTransparentColorSpaceGammaForwardPass.Setup(colorAttachment, m_ActiveCameraDepthAttachment);
                    EnqueuePass(m_RenderTransparentColorSpaceGammaForwardPass);
                }

                void EnqueueTransparentNormalPass()
                {
                ...
                if (cameraData.isSceneViewCamera)
                {
                    ...
                    if (m_mixGammaColorSpace && cameraData.forceGammaColorSpace)
                    {
                        ...
                        EnqueuePass(m_TransparentLinearToSRGBConversionSceneView);
                        m_TransparentSRGBToLinearConversionSceneView.Setup(m_GammaColorTexture, m_ActiveCameraColorAttachment);
                        EnqueuePass(m_TransparentSRGBToLinearConversionSceneView);
                    }
                }
                else
#endif
...
// extensions modify end;
                ...
                {
// extensions modify begin;
                    m_FinalBlitPass.Setup(cameraTargetDescriptor, sourceForFinalPass, m_mixGammaColorSpace && cameraData.requireSRGBToLinearConversion);
// extensions modify end;
...
// extensions modify begin;
            else if (m_mixGammaColorSpace && cameraData.requireLinearToSRGBConversion)
            {
                m_LinearToSRGBConversion.Setup(m_ActiveCameraColorAttachment, m_GammaColorTexture);
                EnqueuePass(m_LinearToSRGBConversion);
            }
// extensions modify end;
#if UNITY_EDITOR
