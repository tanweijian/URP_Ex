...
// extensions modify begin;
    internal class DrawObjectsGammaSpacePass : DrawObjectsPass
    {
        private RTHandle colorTargetIndentifiers;
        private RTHandle depthTargetIndentifiers;

        public DrawObjectsGammaSpacePass(string profilerTag, bool opaque, RenderPassEvent evt, RenderQueueRange renderQueueRange, LayerMask layerMask, StencilState stencilState, int stencilReference) : base(profilerTag, opaque, evt, renderQueueRange, layerMask, stencilState, stencilReference)
        {
        }

        public void Setup(RTHandle inColorAttachment, RTHandle inDepthAttachment)
        {
            colorTargetIndentifiers = inColorAttachment;
            depthTargetIndentifiers = inDepthAttachment;
        }

        public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
        {
            ConfigureTarget(colorTargetIndentifiers, depthTargetIndentifiers);
        }
